...
Deploy the accompanying canonical lookup and merged-record restore/delete guards before releasing the sync holds. These preserve old import/email lookup identities but return KEEP, and refuse ordinary edits or restoration of FOLD and independent deletion while KEEP exists. For a later legitimate erasure, delete/scrub KEEP first and then its same-business aliases; a sweep that reaches FOLD first must retry it after KEEP is erased. This repair itself never hard-deletes either record. Only after successful live verification, add the `migrations-applied` PR label and report that holds can be released. Committing, merging, and passing local tests do not execute the production repair.
