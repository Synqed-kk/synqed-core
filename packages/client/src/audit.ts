...
export class AuditClient {
  ...
  async list(options?: ListAuditOptions): Promise<ListAuditResponse> {
    ...
    if (options?.severity) params.set('severity', Array.isArray(options.severity) ? options.severity.join(',') : options.severity)
    if (options?.action) params.set('action', options.action)
    if (options?.store_id) params.set('store_id', options.store_id)
    if (options?.exclude_views) params.set('exclude_views', 'true')
