...
import { MenuBandInvalidError, MenuQualificationNotFoundError } from '../services/menu.service.js'
...
menuRoutes.post('/', async (c) => {
  ...
  try {
    ...
    if (err instanceof MenuQualificationNotFoundError) return c.json({ error: err.message }, 404)
    throw err
  }
})
