...
syncRoutes.get('/:provider/config', async (c) => {
  ...
  const config = await syncService.getConfig(tenantId, providerParsed.data)
...
syncRoutes.put('/:provider/config', async (c) => {
  ...
  const config = await syncService.upsertConfig(tenantId, providerParsed.data, parsed.data)
  return c.json(config)
})
...
syncRoutes.post('/:provider/run', async (c) => {
  ...
  try {
    const result = await syncService.runSyncForTenant(tenantId, providerParsed.data)
    return c.json(result)
  } catch (err) {
