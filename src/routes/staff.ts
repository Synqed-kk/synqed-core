...
staffRoutes.get('/', async (c) => {
  ...
  const result = await staffService.listStaff(businessId, parsed.data)
  return c.json(result)
})

staffRoutes.get('/:id', async (c) => {
  const businessId = c.get('businessId')
  const staff = await staffService.getStaff(businessId, c.req.param('id'))
...
staffRoutes.delete('/:id', async (c) => {
  const businessId = c.get('businessId')
  try {
    await staffService.deleteStaff(businessId, c.req.param('id'))
...
staffRoutes.post('/:id/pin/verify', async (c) => {
  ...
  if (!parsed.success) return c.json({ error: parsed.error.issues[0].message }, 400)
  try {
    const result = await staffService.verifyPin(businessId, id, parsed.data.pin)
    return c.json(result)
  } catch (err) {
    if (err instanceof Error && err.message === 'Staff not found') {
      return c.json({ error: 'Staff not found' }, 404)
...
staffRoutes.get('/:id/pin', async (c) => {
  ...
  const id = c.req.param('id')
  try {
    const result = await staffService.hasPin(businessId, id)
    return c.json(result)
  } catch (err) {
    if (err instanceof Error && err.message === 'Staff not found') {
      return c.json({ error: 'Staff not found' }, 404)
...
staffRoutes.post('/:id/avatar', async (c) => {
  ...
  try {
    const result = await staffService.uploadAvatar(businessId, id, file)
    return c.json(result)
  } catch (err) {
    if (err instanceof Error && err.message === 'Staff not found') {
